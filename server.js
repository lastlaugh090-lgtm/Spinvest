...
app.post('/api/deposits', auth, async (req, res) => {
  try {
    const { kind, item_id, amount, proof_image } = req.body;
    const user = await User.findOne({ id: req.user.id });
    if (!user) return res.status(404).json({ error: 'Not found' });

    let finalAmount = Number(amount || 0);
    let finalItem = item_id || '';
    if (kind === 'nft') {
      const nft = NFTS.find((n) => n.id === item_id);
      if (!nft) return res.status(400).json({ error: 'Invalid VIP' });
      finalAmount = nft.price;
      finalItem = nft.id;
    } else if (kind === 'invest') {
      if (!finalAmount || finalAmount < INVEST_MIN) {
        return res.status(400).json({ error: 'Min invest ₦' + INVEST_MIN });
      }
      finalItem = 'invest';
    } else if (kind === 'trust_boost') {
      if (!finalAmount || finalAmount < 1000) return res.status(400).json({ error: 'Invalid boost amount' });
    } else {
    ...
    if (proof_image && typeof proof_image === 'string') {
      if (proof_image.length > 900000) return res.status(400).json({ error: 'Screenshot too large (max ~600KB)' });
      proof = proof_image;
    }
    ...
    res.json({ id: dep.id, amount: finalAmount, kind, status: 'pending', has_proof: !!proof });
  } catch (e) {
    console.error(e);
...
// —— Tasks ——
const TASK_POOL = [
  { id: 1, title: 'Browse Amazon', desc: 'Open Amazon and scroll products for 25 seconds.', link: 'https://www.amazon.com', linkLabel: 'Open Amazon', timerSec: 25 },
  { id: 2, title: 'Check BBC News', desc: 'Read a BBC Africa headline for 30 seconds.', link: 'https://www.bbc.com/news/world/africa', linkLabel: 'Open BBC Africa', timerSec: 30 },
  { id: 3, title: 'Visit Wikipedia', desc: 'Open a random Wikipedia article and read for 25s.', link: 'https://en.wikipedia.org/wiki/Special:Random', linkLabel: 'Open Wikipedia', timerSec: 25 },
  { id: 4, title: 'Explore YouTube', desc: 'Open YouTube trending and watch the page for 30s.', link: 'https://www.youtube.com/feed/trending', linkLabel: 'Open YouTube', timerSec: 30 },
  { id: 5, title: 'Visit Coursera', desc: 'Browse free courses on Coursera for 25 seconds.', link: 'https://www.coursera.org/courses', linkLabel: 'Open Coursera', timerSec: 25 },
  { id: 6, title: 'Check Investing.com', desc: 'View market overview on Investing.com (30s).', link: 'https://www.investing.com', linkLabel: 'Open Investing.com', timerSec: 30 },
  { id: 7, title: 'Browse Medium', desc: 'Open Medium and skim one story for 25 seconds.', link: 'https://medium.com', linkLabel: 'Open Medium', timerSec: 25 },
  { id: 8, title: 'Visit Unsplash', desc: 'Browse photos on Unsplash for 25 seconds.', link: 'https://unsplash.com', linkLabel: 'Open Unsplash', timerSec: 25 },
  { id: 9, title: 'Check Weather', desc: 'Open AccuWeather and check a forecast (20s).', link: 'https://www.accuweather.com', linkLabel: 'Open AccuWeather', timerSec: 20 },
  { id: 10, title: 'Visit TED Ideas', desc: 'Browse TED ideas page for 25 seconds.', link: 'https://www.ted.com/topics', linkLabel: 'Open TED', timerSec: 25 },
  { id: 11, title: 'Explore GitHub', desc: 'Open GitHub trending repositories (25s).', link: 'https://github.com/trending', linkLabel: 'Open GitHub', timerSec: 25 },
  { id: 12, title: 'Read TechCrunch', desc: 'Skim a TechCrunch article for 30 seconds.', link: 'https://techcrunch.com', linkLabel: 'Open TechCrunch', timerSec: 30 }
];
...
app.post('/api/tasks/complete', auth, async (req, res) => {
  try {
    ...
    const started_at = Number(req.body.started_at || 0);
    const user = await User.findOne({ id: req.user.id });
    if (!user) return res.status(404).json({ error: 'Not found' });
    const nft = NFTS.find((n) => n.id === user.nft_id);
    if (!nft) return res.status(400).json({ error: 'Buy a VIP pass to unlock tasks' });
    const task = TASK_POOL.find((t) => t.id === task_id);
    if (!task || task_id > nft.dailyTasks) {
      return res.status(400).json({ error: 'Invalid task for your VIP level' });
    }
    const need = Number(task.timerSec || 25);
    if (!started_at || Date.now() - started_at < (need - 2) * 1000) {
      return res.status(400).json({
        error: 'Wait for the timer to finish (open the link and stay at least ' + need + 's)'
      });
    }
    const day = todayKey();
    ...
    res.json({ ok: true, reward, balance: user.balance, message: '₦' + reward + ' added' });
  } catch (e) {
    console.error(e);
    res.status(500).json({ error: e.message || 'Task failed' });
  }
});
...
app.post('/api/admin/login', (req, res) => {
  const pwd = String((req.body && req.body.password) || '').trim();
  if (!pwd || pwd !== String(ADMIN_PASSWORD).trim()) {
...
app.get('/api/admin/deposits', adminAuth, async (req, res) => {
  // returns pending deposits; proof available as has_proof
  ...
  for (const d of list) {
    ...
    out.push({
      ...
      amount: d.amount,
      status: d.status,
      has_proof: !!(d.proof_image && d.proof_image.length > 20),
      proof_image: d.proof_image && d.proof_image.length < 400000 ? d.proof_image : (d.proof_image ? d.proof_image.slice(0, 0) : ''),
